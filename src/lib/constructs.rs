...
impl<'a> Construct<'a> {
    pub fn parse(construct: &'a str, lines: &mut Iter<&'a str>, user_funcs: &HashMap<&'a str, UserFunction<'a>>) -> Option<Construct<'a>> {
        ...
        if let Some(capture) = if_regex.captures(&construct) {
            ...
            let expression = Expression::parse(expression, user_funcs).unwrap();
    ...
    pub fn apply(&self, data_store: &mut DataStore<'a>, user_funcs: &HashMap<&'a str, UserFunction<'a>>) {
        match self {
            Construct::If(expr, sub) => {
                if Expression::evaluate(expr, data_store, user_funcs).unwrap() != 0 {
                    sub.run_with(data_store);
                }
            }
            Construct::While(expr, sub) => {
                while Expression::evaluate(expr, data_store, user_funcs).unwrap() != 0 {
                    sub.run_with(data_store);
                }
            }
            Construct::For(var, start, end, sub) => {
                data_store.expand();
                let start = Expression::evaluate(start, data_store, user_funcs).unwrap();
                let end = Expression::evaluate(end, data_store, user_funcs).unwrap();
                for i in start..end {
